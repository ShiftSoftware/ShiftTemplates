...
**Proposed 2026-09-18; Stages 0–2 built, nothing flipped yet.** The next mapping step is to retire `ShiftEntity.SourceGenerator` and have ShiftMapper declare and generate the four repository maps (nested children included) automatically from `ShiftRepository<,,,>` type arguments and the endpoint attributes — no mapping attributes for the programmer (the three `ShiftEntityMapper*` attributes and `UseGeneratedMapper = true` are deleted), configuration either in the repository as `o.Mapping(m => m.List.ForMember(...))` in ShiftMapper's vocabulary or in an ordinary `ShiftMapperBase` class that overrides the repository for the pair it declares, and the maps usable anywhere through `Mapper`/`IMapper`. Plan: [`docs/plans/repository-mapping-on-shiftmapper/`](docs/plans/repository-mapping-on-shiftmapper/) — start at `README.md`, track in `STATUS.md`. **Stage 0 is done (2026-09-18):** `StockPlusPlus.Test/Tests/RepositoryMappingParityTests.cs` pins all 23 triples' four directions against frozen goldens in `Tests/Parity/RepositoryMapping/Goldens/` captured from the old generator — never regenerate them casually (`SHIFT_TEST_CAPTURE_REPOSITORY_MAPPING_GOLDENS=1` is the switch, and after Stage 3 it captures the implementation under test); the inventory and SHENGEN baseline are in `05-inventory.md`. **Stage 1 is implemented but unreleased (2026-09-18):** ShiftMapper 0.3.0 in the sibling checkout carries implicit maps (`ShiftMapperDeclaresMap`), configuration surfaces (`ShiftMapperConfigurationSurface`, `IMapper.Configure`), `IgnoreMember` rules, `ForEachElement` conventions and SM0047–SM0053; the declaration contract is now 3, so the first framework release after this must be `release-all` (or `release-shiftmapper` first) — `ShiftMapperVersion` already says 0.3.0. **Stage 2 is done (2026-09-19), additively:** `ShiftEntity.Core` and `ShiftEntity.EFCore` reference ShiftMapper (library + generator, the CosmosDbReplication pattern; Model stays `netstandard2.0` and out); the rules pack is `ShiftEntity.Core/Mapping/ShiftEntityConversions.cs`, the tag maps `ShiftEntity.EFCore/Tagging/ShiftEntityFrameworkMaps.cs`, the markers sit on `ShiftRepository<,,,>` and on the two built-in-repository endpoint attributes (not the `WithMapper` or custom-repository ones), `o.Mapping(m => …)` exists on `ShiftRepositoryOptions`, and `ShiftRepository.InitCommon` resolves `ShiftMapperEntityMapper<E,L,V>` over the host's `IMapper` FOURTH — behind the DI mapper and the registry — so nothing the sample serves has changed yet; `RegisterShiftRepositories` registers ShiftMapper (EFCore's own generated mapper with the shared pack, plus every scanned assembly's through the new `AddShiftMapper(Assembly)`), so the sample host has no `AddShiftMapper()` line and needs none. `StockPlusPlus.Data`'s build now declares its 40 maps with nothing written in it, and `StockPlusPlus.Test/Tests/RepositoryMappingShiftMapperDiffTests.cs` diffs all 23 goldens through ShiftMapper: the 10 un-customized triples match, the rest wait for Stage 3's `o.Mapping` migration (the `Report` theories print the worklist). ShiftMapper 0.3.0 grew seven small things for it (see the plan's Stage 2 log). It has no `.shift` mirror; the `.shift` mapping plan only points here. Until its Stage 3 lands, everything in "Active Work: Mapping Abstraction" still describes the code as it is.
