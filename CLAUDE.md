...
**Cosmos replication maps through ShiftMapper.** The mapping delegate on `Replicate` / `UpdateReference` / `UpdatePropertyReference` (both pipelines in `ShiftEntity.CosmosDbReplication`) is optional; a call site that omits it maps through the host's registered `IShiftMapper`, resolved by `ReplicationMapper.cs` ONCE per registered action and ABOVE the row loop, with the pair checked via `CanMap` — a missing mapper or pair throws out of the run rather than surfacing as permanently-dirty rows under a clean watermark (which is how the old AutoMapper fallback failed; never move the resolution inside the per-row catch). ShiftIdentity's 19 replication pairs live in `ShiftIdentity.Data/Replication/IdentityReplicationProfile.cs` (a `ShiftMapperProfile`), carried by the ready-made `ShiftIdentityReplicationMapper`; `SetUpAllIdentityReplications` (Dashboard.AspNetCore), `ReplicateAllAsync` (AzureFunctions) and the template's `UtilityController.ReplicateAll` pass no delegates. **The mapper is registered by the identity registrations themselves** — `AddShiftIdentityDashboard<TDbContext>()` (API side) and the Functions worker's `AddShiftIdentity(issuer, key)` (catch-up side) both call the idempotent `AddShiftIdentityReplicationMapper()` — so the template hosts carry no line for it; it cannot ride on `SetUpAllIdentityReplications` because the trigger's options callback runs per scope over an already-built container. A host that wires identity replication without either identity registration calls `AddShiftIdentityReplicationMapper()` itself. The documents are pinned by the 24 frozen goldens in `StockPlusPlus.Test/Tests/ReplicationMappingParityTests.cs` plus `IdentityReplicationMapperRegistrationTests.cs`; the profile's transcription rules (ignore `ID` on every map, ternaries for navigation reads, `"0"` for a key behind a null navigation, `BranchID` ignored on the merge maps) are documented in the profile itself. The repository triple (`IShiftEntityMapper`, the ShiftEntity generator) does NOT use ShiftMapper — do not widen its use there without being asked.
