...
- **Invoice, api/country-generated** — automatic maps CUSTOMIZED where the repository is configured: `Repositories/InvoiceRepository.cs` writes `o.Mapping(m => m.List.ForMember(d => d.Total, opt => opt.MapFrom(i => i.InvoiceLines.Sum(l => l.Price))))` (children nest with nothing written; SM0049 notes the write direction replaces them, which `UpsertAsync` owns); `Country.ConfigureRepository` writes the same from the entity (`context.Options.Mapping(...)`) — one DTO type as list and view there is ONE map (Q15).
- **ProductBrand** — a MAPPER CLASS: `Mappers/ProductBrandMapper.cs` is an ordinary `ShiftMapperBase` declaring `CreateMap<ProductBrand, ProductBrandListDTO>().ForMember(Code…)` and `CreateMap<ProductBrandDTO, ProductBrand>().AfterMap(trim)`; those replace the automatic maps for their pairs (SM0047, Info), the other two pairs stay automatic, nothing is plugged into the repository.
...
- The maps are ordinary ShiftMapper maps in `StockPlusPlus.Data`'s generated mapper (40 of them: 10 triples + nested), registered by `RegisterShiftRepositories(typeof(Marker).Assembly)`; any service injects `Mapper` (typed) or `IMapper`. A customization written in a repository reaches a map used FIRST in a scope through the DI pull (`ShiftEntityConfiguratorResolver`; an entity configurator is reached through the built-in repository closed over the host's `DbContextOptions.ContextType`).
- Tests: `Tests/ManualMappingTests.cs` (Product/ProductCategory/Invoice end-to-end), `Tests/SourceGeneratedMappingTests.cs` (`MapperClassDoorTests`, `AutomaticMappingTests`, the end-to-end `SourceGeneratedMappingTests`, `RepositoryConfigurationTests`), `Tests/DeepMappingTests.cs`, `Tests/DeepListMappingTests.cs`, `Tests/DeepListTranslationTests.cs` (SQL); `Tests/MappingPOC/` holds POC comparison files (Manual/Mapperly/Mapster, kept only as a one-off comparison — none of them is a supported framework mapping strategy).

The nesting depth is `Nested = 10` on the framework's markers, capped per repository with `m.Nested(n)` inside `Mapping(...)`; a member is dropped with `m.View.ForMember(d => d.X, opt => opt.Ignore())` (one line per direction it exists in). There are no mapping attributes.
...
- `ShiftEntity.EFCore/Mapping/` — `ShiftMapperEntityMapper<E,L,V>` (the `IShiftEntityMapper` adapter over the host's `IMapper`), `ShiftEntityMapping<E,L,V>` (the `o.Mapping(m => …)` surface), `ShiftEntityConfiguratorResolver` (how ShiftMapper pulls a repository's configuration when a map is used first)
