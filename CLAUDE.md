...
**Cosmos replication maps through ShiftMapper.** The mapping delegate on `Replicate` / `UpdateReference` / `UpdatePropertyReference` (both pipelines in `ShiftEntity.CosmosDbReplication`) is optional; a call site that omits it maps through the host's registered `IShiftMapper`, resolved by `ReplicationMapper.cs` ONCE per registered action and ABOVE the row loop, with the pair checked via `CanMap` — a missing mapper or pair throws out of the run rather than surfacing as permanently-dirty rows under a clean watermark (which is how the old AutoMapper fallback failed; never move the resolution inside the per-row catch). ShiftIdentity's 19 replication pairs are declared directly in `ShiftIdentity.Data/Replication/ShiftIdentityReplicationMapper.cs` (a partial `ShiftMapperBase`; ShiftMapper 0.2.0 removed profiles — an included mapper IS the shareable unit, and rules go in a `ShiftMapperConversions` pack, of which identity has none); `SetUpAllIdentityReplications` (Dashboard.AspNetCore), `ReplicateAllAsync` (AzureFunctions) and the template's `UtilityController.ReplicateAll` pass no delegates. **The mapper is registered by the identity registrations themselves** — `AddShiftIdentityDashboard<TDbContext>()` (API side) and the Functions worker's `AddShiftIdentity(issuer, key)` (catch-up side) both call the idempotent `AddShiftIdentityReplicationMapper()` — so the template hosts carry no line for it; it cannot ride on `SetUpAllIdentityReplications` because the trigger's options callback runs per scope over an already-built container. That call is ShiftMapper's "package registers itself" shape: an inline `services.AddShiftMapper(o => { o.Lifetime = …; o.AddMapper<ShiftIdentityReplicationMapper>(); })` made FROM ShiftIdentity.Data (the mapper's own assembly, so no adapter is needed and a host that registers the mapper itself wins in either order); keep it an inline lambda, not the generic short form, because `Assembly.GetCallingAssembly()` is not inlining-safe across the identity registrations. ShiftMapper now keeps ONE registry per collection and throws on a duplicate registration, so the `services.Any(...)` guard in `AddShiftIdentityReplicationMapper()` is load-bearing. `IShiftMapper` is a single door (the mapper, or a `CompositeShiftMapper` that dispatches each pair to the FIRST registered mapper declaring it); `ReplicationMapper.cs` resolves that one door and asks it `CanMap` — it no longer enumerates mappers or picks the last. A host with its own mapper writes `IncludeMapper<ShiftIdentityReplicationMapper>()`; re-declaring one of the 19 pairs in a second registered mapper is refused (SM0040 / startup). A host that wires identity replication without either identity registration calls `AddShiftIdentityReplicationMapper()` itself. The documents are pinned by the 24 frozen goldens in `StockPlusPlus.Test/Tests/ReplicationMappingParityTests.cs` plus `IdentityReplicationMapperRegistrationTests.cs` (which also cross-checks the pairs against the `ShiftMapperDeclaredMap` metadata by `DeclaredBy`); the mapper's transcription rules (ignore `ID` on every map, ternaries for navigation reads, `"0"` for a key behind a null navigation, `BranchID` ignored on the merge maps) are documented in the mapper itself. The repository triple (`IShiftEntityMapper`, the ShiftEntity generator) does NOT use ShiftMapper — do not widen its use there without being asked.
...
Azure DevOps pipeline (`azure-pipeline.yml`) triggers on `release*` tags. It clones all framework sibling repos, builds everything, runs tests, then packs and publishes NuGet packages. On `release-all` and `release-framework` tags only, before anything is packed, `.pipelines/identity-checks.steps.yml` runs four identity gates against a disposable SQL Server container (the other tags publish no identity package and skip the whole block at template-expansion time): `identity-policy` (database-free, `Category=Policy`), `identity-sql` (`Category=Sql`), `identity-ui` (bunit, `Category=Ui`) and `identity-consumer-contract` (`Category=IdentityConsumer` in `StockPlusPlus.Test`), plus an opt-in `identity-long-running` gate (`Category=LongRunning`, the real-clock response-floor evidence) that runs only when the tag name contains `with-long-running-tests`, for example `release-framework-with-long-running-tests`; those tests also skip themselves unless `SHIFT_IDENTITY_LONG_RUNNING=1`, which that gate sets and a local run can set. Every `Http`-tagged identity class is also `Sql`-tagged, so the policy gate must not filter on `Http` or the SQL suite runs twice. Tag naming controls what gets published: `release-all`, `release-framework`, `release-typeauth`, `release-aspNetCore-authorization`, `release-shiftmapper`. `release-all` publishes every group; the specific tags publish only their own. The ShiftMapper group runs both of its test projects, packs `ShiftSoftware.ShiftMapper` (one package carrying the library and the generator) into its own staging folder, verifies both halves are inside the `.nupkg`, and pushes with `-SkipDuplicate` so a `release-all` after a `release-shiftmapper` of the same version does not fail.
