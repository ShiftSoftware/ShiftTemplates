...
| B2 `dotnet new shiftentity` emits a mapper | ➖ | **Superseded 2026-09-20** by the repository-mapping-on-ShiftMapper work (`docs/plans/repository-mapping-on-shiftmapper/`, Stage 4b): the maps are automatic and a project has ONE mapper class for its customizations (`Mappers/StockPlusPlusMapper.cs` in the sample, `<Project>Mapper.cs` in a generated project), so the item template no longer scaffolds a per-entity mapper — `ProductBrandMapper.cs` left its `include` + `rename` lists (the file is gone; its maps live in the one class) and the scaffolded repository's comment points at the project's class. What stays from this step: the project template's widened `Mappers/` modifier (a sample-app-free project ships no mapper class; the programmer creates the one class when a customization is first needed) and the `ProductBrandForm.razor` restructuring. The Builder run this row owed is still owed, for the same reason as before. |
