...
> 🔒 **Detail withheld from this public copy.** The specific request path and field combination that make
> this exploitable are recorded only in the private `.shift` copy of this document, and stay there until the
> change ships. Work the decision from the summary below; read `.shift` for the reproduction.

`MapToEntity` will stop writing `ID`, `IsDeleted` and the four audit fields, which AutoMapper's unguarded
`ReverseMap()` **did** write. That is strictly a narrowing — and a security-positive one: today those fields
are writable by the client on at least one identity upsert path, which lets a caller change record state that
should be server-controlled.
...
probably correct given what is reachable today.
